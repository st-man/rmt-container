...
RUN crontab -l | { cat; echo "00 00 1 * * rmt-cli systems list > ~/systems_'$(date +"%d-%m-%Y")'.txt && rsync -aqze '"ssh -p 22"' --remove-source-files ~/systems* ${RSYNC_USER}@${RMT_REMOTE_HOST}:~"; } | crontab -
